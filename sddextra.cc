...
SddLiteral 
SddExtra::vtree_highest_var(Vtree* tree) const
{
	if (tree == 0) return 0;
...
void 
SddExtra::sdd_mit_init_prime(struct sdd_mit_master* mas, Vtree* tree, SddManager* Manager) const
{
...
void 
SddExtra::sdd_mit_init_sub(struct sdd_mit_master* mas, Vtree* tree, SddManager* Manager) const
{
	if (tree == 0 || sdd_vtree_is_leaf(tree)) return;
	struct sdd_model_iterator* it = &(mas->nodes[sdd_vtree_position(tree)]);
	Vtree* root_tree = (it->root == 0) ? 0 : sdd_vtree_of(it->root);
...
void 
SddExtra::sdd_mit_skip_false_prime(struct sdd_model_iterator* it) const
{
...
void 
SddExtra::sdd_get_iterator_rec(struct sdd_mit_master* mas, SddNode* root, Vtree* tree, SddManager* Manager) const
{
	struct sdd_model_iterator* it = &(mas->nodes[sdd_vtree_position(tree)]);
	it->root = root;
	it->i = 0;
...
struct sdd_mit_master 
SddExtra::sdd_get_iterator(SddNode* set, SddManager* Manager) const
{
	...
	for (int i = 0; i < sdd_manager_var_count(Manager); i++) 
		mas.e[i] = 0;
	mas.finished = 0;
	mas.nodes = (struct sdd_model_iterator*) malloc(sizeof(struct sdd_model_iterator) * 2*sdd_manager_var_count(Manager));
	sdd_get_iterator_rec(&mas, set, sdd_manager_vtree(Manager), Manager);
	return mas;
}

void 
SddExtra::sdd_mit_free(struct sdd_mit_master mas) const
{
	free(mas.e);
	free(mas.nodes);
}
...
void 
SddExtra::sdd_next_model_rec(struct sdd_mit_master* mas, Vtree* tree, SddManager* Manager) const
{
...
void 
SddExtra::sdd_next_model(struct sdd_mit_master* mas, SddManager* Manager) const
{
...
void 
SddExtra::set_enum(SddNode* set, SddManager* Manager, void (*funcptr)(int*)) const
{
