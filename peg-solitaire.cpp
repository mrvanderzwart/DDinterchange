...
SddManager* SddExtra::Manager = sdd_manager_create(2*((7*7)-(4*4)), 0);
...
void printboardsSDD(int *e, std::vector<SddLiteral> set, void *ctx)
{
  printf("--------------------\n");
  ...
  for (int i = 0; i < N; i++)
  {
    for (int j = 0; j < N; j++)
    {
      ...
      {
        SddLiteral var = Game->Coor2Var[i][j];
...
PegSolitaire::PrintBoards(dd &D, std::string name) 
{
  ...
  SddExtra::set_enum(D.GetDD(), SourceVars.toVector(), printboardsSDD, this); //sdd
}
