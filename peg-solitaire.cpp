...
SddManager* SddSatEnum::Manager = sdd_manager_create(2*((3*3)-(2*2)), 0);
...
PegSolitaire::CreateInitialBoard()
{
  ...
  for (int i = 0; i < N; i++) 
  {
    for (int j = 0; j < N; j++) 
    {
      ...
      if (BOARD[i][j] == EE)
        InitialBoard &= !DecisionDiagram::ddVar(square);
      else 
        InitialBoard &= DecisionDiagram::ddVar(square);
    }
  }
...
void PegSolitaire::CreateWinningBoards() 
{
  ...
  for (ddvar square : SourceVars.toVector()) 
  {
    ...
    for (ddvar square2 : SourceVars.toVector()) 
    {
      if (square2 == square) 
      {
        Board &= DecisionDiagram::ddVar(square2);
      }
      else 
        Board &= !DecisionDiagram::ddVar(square2);
    }
...
void PegSolitaire::CreateMoveRelation()
{
  for (ddset &Triple : Triples)
  {
    ...
    dd move1 =  DecisionDiagram::ddVar(V[0]) & !DecisionDiagram::ddVar(V[0]+1) &
                DecisionDiagram::ddVar(V[1]) & !DecisionDiagram::ddVar(V[1]+1) &
               !DecisionDiagram::ddVar(V[2]) &  DecisionDiagram::ddVar(V[2]+1);
               
    Partial.MoveRelation |= move1;
    
    dd move2 = (!DecisionDiagram::ddVar(V[0])) &  DecisionDiagram::ddVar(V[0]+1) &
                 DecisionDiagram::ddVar(V[1])  & !DecisionDiagram::ddVar(V[1]+1) &
                 DecisionDiagram::ddVar(V[2])  & !DecisionDiagram::ddVar(V[2]+1);
  ...
  for (PartialRelation& Partial : Relations)
  {
    ...
    for (ddvar var : SourceVars.toVector())
    {
      ...
      Rel &= (DecisionDiagram::ddVar(var) - DecisionDiagram::ddVar(var+1)); 
    }
    MoveRelation |= Rel;
