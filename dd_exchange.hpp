...
{ 
  ...
  static SddManager *Manager;
    ...
    Sdd() { sdd = sdd_manager_false(SddExtra::Manager); sdd_ref(sdd, SddExtra::Manager); }
    Sdd(const SDD from) : sdd(from) { sdd_ref(from, SddExtra::Manager); }
    Sdd(const SddLiteral from) { sdd = sdd_manager_literal(from, SddExtra::Manager); sdd_ref(sdd, SddExtra::Manager); }
    Sdd(const Sdd& from) : sdd(from.sdd) { sdd_ref(from.sdd, SddExtra::Manager); }
