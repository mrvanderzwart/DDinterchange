...
{
  return Sdd(sdd_manager_literal(index, Manager));
}
...
Sdd::operator-(const Sdd& other) const
{
  return Sdd(sdd_disjoin(sdd_conjoin(sdd, other.sdd, Manager), sdd_conjoin(sdd_negate(sdd, Manager), sdd_negate(other.sdd, Manager), Manager), Manager));
}
...
{ 
	static int init = 0;
	if (init < sdd_manager_var_count(Sdd::Manager))
	{
		init++;
		variable++;
	}
