...
class SddExtra
{
  ...
  public:
    ...
    static struct sdd_mit_master sdd_get_iterator(SddNode* set) 
    {
	    ...
	    sdd_get_iterator_rec(&mas, set, sdd_manager_vtree(Manager));
	    return mas;
    }
    ...
    {
    	sdd_next_model_rec(mas, sdd_manager_vtree(Manager));
	    mas->finished = mas->nodes[sdd_vtree_position(sdd_manager_vtree(Manager))].finished;
    }
    
    static void set_enum(SddNode* set, std::vector<SddLiteral> sources, void (*funcptr)(int*,std::vector<SddLiteral>,void*), void *ctx)
    {
